...
export class AuthserviceService {
  ...
  constructor(
    ...
    private readonly administratorModel: typeof Administrator,
  ...
  async validateUser(email: string, pass: string): Promise<any> {
    const profesor = await this.professorModel.findOne({
      where: { correo: email },
    });
    ...
    const administrador = await this.administratorModel.findOne({
      where: { correo: email },
    });
    if (
      administrador &&
      (await bcrypt.compare(pass, administrador.contrasena))
    ) {
      return {
        id: administrador.id,
  ...
  async registerProfessor(
    ...
    const { nombre, correo, rut } = createEmailServerDto;
  ...
  async resetPassword(correo: string): Promise<string> {
    let usuario: Professor | Administrator = await this.professorModel.findOne({
      where: { correo },
    });
  ...
  async changePassword(
    ...
    let usuario =
      (await this.professorModel.findOne({ where: { correo } })) ||
